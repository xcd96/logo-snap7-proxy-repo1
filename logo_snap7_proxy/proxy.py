...
client.connect(LOGO_IP, 0, 1)  # Verbindung zur LOGO!

print("Proxy gestartet und mit LOGO! verbunden")

def set_output(q_num, value):
    # LOGO!-Ausgänge liegen im Prozessabbild Ausgangsbereich (PA)
    # Adresse 0 ist Q1, Adresse 1 ist Q2 usw.
...
def on_message(client_mqtt, userdata, msg):
    print(f"MQTT Message empfangen: {msg.topic} => {msg.payload}")
    try:
        if msg.topic.startswith("logo/q") and msg.topic.endswith("/set"):
            q_num = int(msg.topic.split("/")[1][1:])  # 'q1' -> 1
            if 1 <= q_num <= 9:
